...
#↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓
        ...
        for local_update in range(args.grad_steps_per_iteration):
            ...
            with torch.no_grad():
                next_state_actions, next_state_log_pi, _ = actor.get_action(data.next_obs[:,-1,:])
                qf1_next_target = qf1_target(data.next_obs[:,-1,:], next_state_actions)
                qf2_next_target = qf2_target(data.next_obs[:,-1,:], next_state_actions)
                min_qf_next_target = torch.min(qf1_next_target, qf2_next_target) - alpha * next_state_log_pi
                next_q_value = data.rewards.flatten() + (1 - data.dones.flatten()) * args.gamma * (min_qf_next_target).view(-1)
                # data.dones is "stop_bootstrap", which is computed earlier according to args.bootstrap_at_done

            qf1_a_values = qf1(data.obs[:,-1,:], data.actions).view(-1)
            qf2_a_values = qf2(data.obs[:,-1,:], data.actions).view(-1)
            ...
            if global_update % args.policy_frequency == 0:  # TD 3 Delayed update support
                pi, log_pi, _ = actor.get_action(data.obs[:,-1,:])
                qf1_pi = qf1(data.obs[:,-1,:], pi)
                qf2_pi = qf2(data.obs[:,-1,:], pi)
                ...
                if args.autotune:
                    with torch.no_grad():
                        _, log_pi, _ = actor.get_action(data.obs[:,-1,:])
