...
#↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓
        ...
        for local_step in range(args.steps_per_env):
            ...
            obs2act = smart_slice(global_observations, args.seq_len, tracker)[1]
            ...
            if "final_info" in infos:
                '''
                СЮДА МЫ ПОПАДАЕМ В СЛУЧАЕ ЕСЛИ:
                1) ВСЕ СРЕДЫ ДОСТИГЛИ EP.TIMEOUT
                2) ОДНА ИЗ СРЕД ДОСТИГЛА EP.TIMEOUT
                При этом вектор truncated имеет True на теx индексах сред где случился эп. таймаут
                Вектор terminated при этом ВСЕШДА False
                Сюда мы НЕ попадаем если случился sr_once и _end. Этот if ловит только таймауты
                ВОПРОС: когда он ловил таймаут, что он делает с этой средой? Делает autoreset?  Если да, то тогда надо это учитывать в tracker
                ОТВЕТ: Да, он делает авторесет и начинает шагать с 0 степа. Это надо учитывать в трекере. Более того, надо учитывать что elapsed steps для жтой среды после попадания в этот 
                if становится равным 0. Учесть эти тонкости в трекере
                
                ИДЕЯ: в смарт слайс подавать не индексы трекера а elapsed steps, ведь они ТОЧНО отражают кол-во настепаных шагов И при авторесете который случается при if funal info И при ручном сбросе по достижению саксесса
                    то есть потребности трекинг делать видимо пропаает. Просто в смарт слайсе смотрим, если elapsed_steps < cont: то делаем то что нам надо 
                    
                autoreset аргумент ресетит среду автоматом если она достигла trunc или term. а trunc она достигает при таймауте. 
                '''
                print(f"TRUNC {truncations}")
                print(f"TERM {terminations}")
                print(f"FINAL INFO {infos['final_info']}")
            ...
            if infos['episode']['success_once'].any():
                '''
                Сюда мы попадаем случае если хоть одна из сред словила sr_once (т.е. задание выполнено)
                ОСОБЕННОСТИ:
                trunated всегда равен false
                terminated всегда равен false 
                векора success и success_once равны True по координате где словил success
                авторесета тут нет, поэтому мы должны это сделать сами.
                '''
                print(f"INTERM TRUNC {truncations}")
                print(f"INTERM TERM {terminations}")
                print(f"INTERM INFO {infos}")
